...
def get_compiled_solver(timesteps: int, dt: float, num_obstacles: int):
    ...
    """
    logger.info(f"JIT Compiling TrajOpt for: timesteps={timesteps}, obstacles={num_obstacles}")
...
def _build_world_coll(obstacles: List[ObstacleEntry] | None) -> List[Any]:
    ...
    for obj in obstacles:
        if obj.type == "halfspace":
            world.append(pk.collision.HalfSpace.from_point_and_normal(np.array(obj.point), np.array(obj.normal)))
        elif obj.type == "sphere":
            world.append(pk.collision.Sphere.from_center_and_radius(np.array(obj.center), np.array([obj.radius])))
...
async def solve_trajectory(req: TrajOptRequest):
    ...
    try:
        ...
        return TrajOptResponse(trajectory=traj_np.tolist(), compute_time=duration)

    except Exception as e:
        logger.exception("Trajectory optimization failed")
        raise HTTPException(500, f"Solver error: {e!s}")
